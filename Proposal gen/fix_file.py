...
for i, line in enumerate(lines):
    if line.startswith("class FirmAPIClient:"):
        start_idx = i - 1 # from the comment above
        break

for i in range(start_idx + 1, len(lines)):
    if line.startswith("    @staticmethod") and "@staticmethod" in lines[i] and "_extract_first" in lines[i+1]:
        end_idx = i
        break
...
if start_idx != -1 and end_idx != -1:
    new_lines = lines[:start_idx] + [new_class + "\n"] + lines[end_idx:]
