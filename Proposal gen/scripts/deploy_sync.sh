...
Options:
  --mode <production|inhouse>   Sync mode. production excludes test/example/demo artifacts (default: production).
  ...
  --force-production-cleanup    Force production cleanup even if remote APP_PROFILE is not production (default: enabled)
...
if [[ -f requirements.txt ]]; then
  ...
  pip install -r requirements.txt >/tmp/proposal-gen-pip.log 2>&1 || { cat /tmp/proposal-gen-pip.log; exit 1; }
fi
rm -rf tests test
...
cd "${REMOTE_DIR}"
    ...
    rm -f db.csv
    rm -rf tests test examples generated
