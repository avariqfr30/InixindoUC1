...
class ProposalGenerator:
    ...
    def _max_words(chap: Dict[str, Any]) -> int:
        return int(ProposalGenerator._target_words(chap) * 1.2)
        ...
        for i, chap in enumerate(UNIVERSAL_STRUCTURE):
            ...
            if ctx['success']:
                try:
                    ...
                    if self._word_count(content) < int(target_words * 0.85):
                        expand = self.ollama.chat(
                            model=LLM_MODEL,
                            messages=[
                                {'role': 'system', 'content': ctx['prompt']},
                                {'role': 'user', 'content': (
                                    f"Revise and expand {chap['title']} to be more detailed. Minimum {target_words} words dengan narasi yang padat dan relevan. "
