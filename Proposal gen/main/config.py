...
def _load_csv_set(name: str) -> set[str]:
...
FIRM_API_TIMEOUT_SECONDS = max(3, _int_env("FIRM_API_TIMEOUT_SECONDS", 8))
...
MAX_ACTIVE_GENERATIONS = max(1, _int_env("MAX_ACTIVE_GENERATIONS", 2))
MAX_GENERATION_BACKLOG = max(MAX_ACTIVE_GENERATIONS, _int_env("MAX_GENERATION_BACKLOG", 18))
JOB_RETENTION_SECONDS = max(300, _int_env("JOB_RETENTION_SECONDS", 1800))
JOB_POLL_INTERVAL_MS = max(1000, _int_env("JOB_POLL_INTERVAL_MS", 2000))
RESEARCH_CACHE_TTL_SECONDS = max(300, _int_env("RESEARCH_CACHE_TTL_SECONDS", 1800))
KB_EMBED_TIMEOUT_SECONDS = max(60, _int_env("KB_EMBED_TIMEOUT_SECONDS", 180))
KB_UPSERT_BATCH_SIZE = max(1, _int_env("KB_UPSERT_BATCH_SIZE", 16))
KB_STARTUP_RETRY_DELAY_SECONDS = max(5, _int_env("KB_STARTUP_RETRY_DELAY_SECONDS", 20))
KB_STARTUP_MAX_RETRIES = max(1, _int_env("KB_STARTUP_MAX_RETRIES", 4))
APP_HOST = os.getenv("APP_HOST", "0.0.0.0").strip() or "0.0.0.0"
APP_PORT = max(1, _int_env("APP_PORT", 5500))
_INSECURE_DEFAULT_KEY = "proposal-gen-dev-secret-change-me"
APP_SECRET_KEY = os.getenv("APP_SECRET_KEY", _INSECURE_DEFAULT_KEY).strip() or _INSECURE_DEFAULT_KEY

def _validate_secret_key(key: str, profile: str) -> None:
    import logging as _log
    if key == _INSECURE_DEFAULT_KEY or len(key) < 32:
        msg = (
            "APP_SECRET_KEY is insecure (using default or too short). "
            "Set a random secret of at least 32 characters in your .env file. "
            "Generate one with: python -c \"import secrets; print(secrets.token_hex(32))\""
        )
        if profile == "production":
            raise RuntimeError(msg)
        _log.getLogger(__name__).warning("⚠ SECURITY: %s", msg)

_validate_secret_key(APP_SECRET_KEY, APP_PROFILE)

_raw_cookie_secure = os.getenv("SESSION_COOKIE_SECURE", "").strip().lower()
SESSION_COOKIE_SECURE = (
    _raw_cookie_secure in {"1", "true", "yes", "on"}
    if _raw_cookie_secure
    else (APP_PROFILE == "production")
)

CORS_ALLOWED_ORIGINS = [
    o.strip()
    for o in os.getenv("CORS_ALLOWED_ORIGINS", "http://localhost:5500,http://127.0.0.1:5500").split(",")
    if o.strip()
]
SESSION_IDLE_TIMEOUT_MINUTES = max(5, _int_env("SESSION_IDLE_TIMEOUT_MINUTES", 45))
SESSION_ABSOLUTE_TIMEOUT_HOURS = max(1, _int_env("SESSION_ABSOLUTE_TIMEOUT_HOURS", 12))
SESSION_TOUCH_INTERVAL_SECONDS = max(5, _int_env("SESSION_TOUCH_INTERVAL_SECONDS", 20))
AUTH_FORCE_SINGLE_SESSION = os.getenv("AUTH_FORCE_SINGLE_SESSION", "true").strip().lower() in {"1", "true", "yes", "on"}
AUTH_MAX_SESSIONS_PER_USER = max(1, _int_env("AUTH_MAX_SESSIONS_PER_USER", 2))
AUTH_MAX_GLOBAL_ACTIVE_SESSIONS = max(1, _int_env("AUTH_MAX_GLOBAL_ACTIVE_SESSIONS", 30))
LOGIN_RATE_LIMIT_WINDOW_SECONDS = max(30, _int_env("LOGIN_RATE_LIMIT_WINDOW_SECONDS", 300))
LOGIN_RATE_LIMIT_MAX_ATTEMPTS = max(1, _int_env("LOGIN_RATE_LIMIT_MAX_ATTEMPTS", 6))
LOGIN_RATE_LIMIT_BLOCK_SECONDS = max(30, _int_env("LOGIN_RATE_LIMIT_BLOCK_SECONDS", 600))
...
ESTIMATED_WORDS_PER_PAGE = max(230, _int_env("ESTIMATED_WORDS_PER_PAGE", 255))
...
SPIRIT_OF_AI_RULES = {
    ...
    "supporting_signals": [
        ...
        "adopsi ai", "otomasi cerdas", "analytics", "data science", "otomasi", "intelligent",
        "analytics platform",
    ],
    "dimension_terms": {
        "business_use_case": [
...
PROPOSAL_MODES = [
    ...
    {
        "value": "kak_response",
        "label": "Proposal Penawaran Tanggapan KAK",
        "description": "Mode tanggapan formal berbasis KAK/TOR dengan struktur isi yang berbeda dari proposal canvasing."
    },
]
