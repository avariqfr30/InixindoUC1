    ...
    def get_framework_catalog(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for record in records:
                ...
                if not item.get("use_cases"):
                    item["use_cases"] = record.get("parent_use_cases") or record.get("proposal_use_cases")
                    ...
                    item["versions"] = [
                        {
                            ...
                            "child_use_cases": record.get("child_use_cases"),
...
class InternalDataClient:
    ...
    def is_using_fallback(self) -> bool:
        if self.internal_data_source != "api":
            return False
        if self.internal_data_fallback != "demo":
            return False
        if self._fallback_triggered:
            return True
        try:
            if not self.api_provider.validate_config().get("ok"):
                return True
            base_url = str(self.api_provider.base_url or "").strip()
            if not base_url or "example.com" in base_url or "localhost" in base_url:
                return True
        except Exception:
            return True
        return False
