"""Shared imports, config, and module-wide utilities for proposal generation."""

import os
from pathlib import Path
import io
import re
import json
import hashlib
import logging
import sqlite3
import shutil
import threading
import time
import uuid
import requests
import pandas as pd
import chromadb
from chromadb.config import Settings
import concurrent.futures
from datetime import datetime
from functools import lru_cache
from typing import Dict, List, Any, Tuple, Optional, Set
from urllib.parse import urljoin, urlparse

import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
import matplotlib.patches as patches
from PIL import Image, ImageDraw, ImageFont, ImageStat

from sqlalchemy import create_engine
import markdown
from bs4 import BeautifulSoup

from docx import Document
from docx.image.exceptions import UnrecognizedImageError
from docx.shared import Pt, RGBColor, Inches, Cm
from docx.enum.text import WD_ALIGN_PARAGRAPH, WD_LINE_SPACING
from docx.oxml import OxmlElement
from docx.oxml.ns import qn
try:
    from pypdf import PdfReader
except Exception:  # pragma: no cover - optional document extraction dependency
    PdfReader = None

from ollama import Client
from chromadb.utils import embedding_functions

from .config import (
    PROJECT_ROOT,
    SERPER_API_KEY, OLLAMA_HOST, LLM_MODEL, EMBED_MODEL,
    PROJECT_DB_PATH, PROJECT_CSV_PATH,
    APP_STATE_DB_PATH, APP_ASSET_ROOT, GENERATED_OUTPUT_DIR,
    MANAGED_INTERNAL_API_CONFIG_PATH,
    VECTOR_STORE_DIR, KB_SYNC_STATE_PATH,
    WRITER_FIRM_NAME, DEFAULT_COLOR, UNIVERSAL_STRUCTURE, KAK_RESPONSE_STRUCTURE,
    PERSONAS, CHAPTER_PERSONA_LENSES, PROPOSAL_SYSTEM_PROMPT,
    PROJECT_DATA_FIELD_ALIASES, PROJECT_STANDARD_FIELD_ALIASES, FIRM_PROFILE_FIELD_ALIASES,
    CLIENT_RELATIONSHIP_FIELD_ALIASES,
    APP_PROFILE, INTERNAL_DATA_SOURCE, INTERNAL_DATA_FALLBACK,
    PROJECT_DATA_SOURCE,
    DEMO_MODE, FIRM_API_URL, FIRM_API_AUTH_MODE, API_AUTH_TOKEN, FIRM_API_USERNAME, FIRM_API_PASSWORD, FIRM_API_CONFIG_FILE,
    FIRM_API_TIMEOUT_SECONDS, FIRM_API_INTEGRATION_MODE, FIRM_API_REQUEST_DEFAULTS,
    FIRM_API_ENDPOINT_CONFIG, FIRM_API_DATASET_CONFIG, FIRM_API_RESOURCE_CONFIG,
    REFERENCE_INTERNAL_ACCOUNT_LOOKUP_MODE, REFERENCE_INTERNAL_ACCOUNT_TEST_EMAILS,
    REFERENCE_INTERNAL_ACCOUNT_LOOKUP_URL, REFERENCE_INTERNAL_ACCOUNT_LOOKUP_USERNAME,
    REFERENCE_INTERNAL_ACCOUNT_LOOKUP_PASSWORD, REFERENCE_INTERNAL_ACCOUNT_LOOKUP_TIMEOUT_SECONDS,
    AUTH_SIGNUP_VERIFICATION_DELIVERY_MODE, AUTH_SIGNUP_VERIFICATION_WEBHOOK_URL,
    AUTH_SIGNUP_VERIFICATION_TIMEOUT_SECONDS,
    MOCK_FIRM_STANDARDS, MOCK_FIRM_PROFILE,
    DATA_ACQUISITION_MODE, GENERATION_PROFILE,
    COMPANY_DNA, VALUE_PLAYBOOK, INDUSTRY_VALUE_DRIVERS, CHAPTER_STANDARD_RULES, SPIRIT_OF_AI_RULES,
    WRITER_FIRM_OFFICE_ADDRESS, WRITER_FIRM_EMAIL, WRITER_FIRM_PHONE, WRITER_FIRM_WHATSAPP,
    WRITER_FIRM_WEBSITE, WRITER_FIRM_LEGAL_NAME, WRITER_FIRM_OPERATING_HOURS,
    WRITER_FIRM_PROFILE_SUMMARY, WRITER_FIRM_CREDENTIAL_HIGHLIGHTS, WRITER_FIRM_SOURCE_URLS,
    WRITER_FIRM_PORTFOLIO,
    RESEARCH_CACHE_TTL_SECONDS,
    KB_EMBED_TIMEOUT_SECONDS, KB_UPSERT_BATCH_SIZE, KB_STARTUP_RETRY_DELAY_SECONDS, KB_STARTUP_MAX_RETRIES,
    MAX_PROPOSAL_PAGES, ESTIMATED_WORDS_PER_PAGE, RESERVED_NON_CONTENT_PAGES, PAGE_SAFETY_BUFFER,
)

logger = logging.getLogger(__name__)


# Wrap Client.chat to build in retry resiliency for transient network/server failures
_original_client_chat = Client.chat

def _chat_with_retry(self, *args, **kwargs) -> Any:
    max_retries = 3
    delay = 1.0
    for attempt in range(max_retries):
        try:
            return _original_client_chat(self, *args, **kwargs)
        except Exception as exc:
            if attempt == max_retries - 1:
                logger.exception("Ollama chat call failed after %d attempts", max_retries)
                raise
            logger.warning(
                "Ollama chat call failed (attempt %d/%d), retrying in %.1fs: %s",
                attempt + 1, max_retries, delay, exc
            )
            time.sleep(delay)
            delay *= 2.0

Client.chat = _chat_with_retry
