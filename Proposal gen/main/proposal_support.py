...
class ProposalSupportMixin:
    ...
    def _individual_expert_capability_rows(
        ...
        for record_index, record in enumerate(ordered_records[:6]):
    ...
    def _render_structured_chapter(
        ...
        if chapter["id"] == "c_11":
            ...
            role_list = "\n".join(
                [
                    "1. **Engagement Director / Project Director** menjaga arah kemitraan, eskalasi, dan keputusan strategis sponsor.",
                    "2. **Project Manager / PMO** mengendalikan ritme kerja, milestone, risiko, dan acceptance deliverable.",
                    "3. **Lead Arsitektur / SPBE** menjaga baseline, target operating model, domain arsitektur, dan konsistensi peta rencana.",
                    "4. **Lead Peta Jalan** menyusun prioritas, dependensi, sequencing, serta rencana transisi yang dapat diputuskan sponsor.",
                    "5. **Governance & Quality Reviewer** memastikan disiplin review, sign-off, quality gate, dan kontrol perubahan.",
                ]
            )
            if ai_mode:
                role_list += "\n6. **Business Translator / AI Lead** menjembatani use case bisnis, integrasi solusi, dan kesiapan adopsi."
            if kak_staffing:
                role_list += "\n" + "\n".join(
                    f"- **Kebutuhan KAK/TOR: {role}** dipetakan ke kapabilitas konsultan dan mekanisme quality review yang relevan."
    ...
    def _ensure_personalization_signals(
        ...
        if terminology:
            ...
            if term_hits < 2:
                additions.append(f"- Istilah kerja yang tetap dijaga mencakup {', '.join(terminology[:3])}.")
